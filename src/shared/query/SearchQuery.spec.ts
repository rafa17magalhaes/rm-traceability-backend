...
describe('SearchQuery', () => {
  ...
  it('should allow chaining using with, equals, notEquals, etc.', () => {
    ...
    expect(query.params[0]).toEqual({ key: "status", operator: ":", value: "active", isOrOpertator: false });
    expect(query.params[1]).toEqual({ key: "role", operator: "!", value: "admin", isOrOpertator: false });
  });
  ...
  it('static from should build a new instance from partial Query', () => {
    const partial = { page: 4, search: "status:active", sort: "name:ASC", size: 100 };
